...
*/
    ...
    private Read readStmt() {
        ...
        // read 문장 파싱 + 식별자 반환
        match(Token.READ);
        Identifier tempId = new Identifier(match(Token.ID) + "_temp");
        match(Token.SEMICOLON);
        return new Read(tempId);
    }
    ...
    private Print printStmt() {
        ...
        if (token == Token.PLUS) {
            match(Token.PLUS);
            newExpr = new Binary(new Operator("+"), newExpr, expr());
        }
        match(Token.SEMICOLON);
        return new Print(newExpr);
    }
...
*/
    ...
    private While whileStmt () {
        ...
        // while 문장 파싱 + 조건식과 문장 반환
        match(Token.WHILE);
        match(Token.LPAREN);
        Expr conditionExpr = expr();
        if (token == Token.AND) {
            match(Token.AND);
            conditionExpr = new Binary(new Operator("&"), conditionExpr, expr());
        }
        match(Token.RPAREN);
        Stmt loopStmt = stmt();
        return new While(conditionExpr, loopStmt);
    }
    ...
    private Expr bexp() {
        // <bexp> -> <aexp> [ (< | <= | > | >= | == | !=) <aexp> ]
        // 비교 연산자를 사용한 이진 표현식을 파싱 + AST 생성
        Expr initialExpr = aexp();
        if (token == Token.LT) {
            Operator op = new Operator(match(token));
            Expr secondExpr = aexp();
            initialExpr = new Binary(op, initialExpr, secondExpr);
        }
        return initialExpr;
    }
    ...
    public static void main(String args[]) {
        ...
        if (args.length == 0) {
            ...
            parser = new Parser(new Lexer("test/" + args[0]));
            do {
                if (parser.token == Token.EOF)
                    break;
